...
scentroid <- apply(t(apply(Sigma, 2, mp, K)), 1, which.max)
t.end1 <- proc.time()
centroid.woc <- as.integer(sbmlogit.remap(scentroid))
centroid.woc.gamma1 <- res$gamma
centroid.woc.eta1 <- res$eta
...
scentroid <- apply(t(apply(Sigma, 2, mp, K)), 1, which.max)
t.end1 <- proc.time()
centroid.ndc <- as.integer(sbmlogit.remap(scentroid))
centroid.ndc.gamma1 <- res$gamma
centroid.ndc.eta1 <- res$eta

# Newman wo degree correction ======================
message('=== KN ===') 
t.start <- proc.time() 
snewman <- sbmlogit.remap(.Call('newman1', g, K))
t.end <- proc.time()
kn.ndc <- as.integer(snewman)

sum(scentroid != ref)
sum(centroid.woc != ref)
sum(centroid.ndc != ref)
sum(kn.ndc != ref)
sum(sbinder != ref)
sum(snewman != ref)
